    ...
    def __init__(self, settings: Settings) -> None:
        ...
        if not api_key:
            raise AIConfigurationError(
                "OPENAI_API_KEY is not configured. Add it to .env before generating a study guide."
            )
        if not settings.ai_model:
            raise AIConfigurationError(
                "AI_MODEL is not configured. Add the model name to .env before generating a study guide."
            )
